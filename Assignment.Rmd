...
## Model #2
A fit against all variables was made and the variance with each factor was analysed. The table is given in the Appendix. Horsepower was selected as the variable to model. This has data that covers a fuller range when the data is split into manual and automatic categories. Weight and displacement look promising BUT there is little overlap when the data is separated into the transmission types. Graphs are given in the Appendix.
